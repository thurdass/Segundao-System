...
public class AdminController {
    ...
    public AdminController(UserRepository u, ActivityRepository a, AnnouncementRepository n) {
        users = u;
        activities = a;
        announcements = n;
    }
    ...
    public UserResponse user(@PathVariable Long id) {
        return UserResponse.from(users.findById(id).orElseThrow(() -> new ResourceNotFoundException("User not found")));
    }
    ...
    public Map<String, Long> dashboard() {
        return Map.of("users", users.count(), "activeUsers", users.findAll().stream().filter(User::isEnabled).count(), "activities", activities.countByActiveTrue(), "announcements", announcements.countByActiveTrue());
    }
}
