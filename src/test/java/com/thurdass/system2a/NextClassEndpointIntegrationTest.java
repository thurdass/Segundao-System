...
class NextClassEndpointIntegrationTest {
    ...
    private static final LocalDate FIXED_DATE = LocalDate.of(2026, 8, 13);
    ...
    void returnsNextOccurrenceAndAllScheduleFields() throws Exception {
        ...
        mockMvc.perform(get("/api/subjects/" + subject.getId() + "/next-class")
                ...
                .andExpect(jsonPath("$.nextClassDate").value(FIXED_DATE.toString()))
    ...
    void skipsPassedCurrentDayAndUsesFollowingWeekWhenNecessary() throws Exception {
        ...
        mockMvc.perform(get("/api/subjects/" + subject.getId() + "/next-class")
                        .header("Authorization", bearer(student)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.nextClassDate").value(FIXED_DATE.plusDays(6).toString()))
                .andExpect(jsonPath("$.dayOfWeek").value("WEDNESDAY"));
    }

    @Test
    void rejectsMissingSubjectScheduleAndDifferentClassroom() throws Exception {
        Classroom ownClassroom = classroom("errors-own");
        Classroom otherClassroom = classroom("errors-other");
        User student = user("errors-user", ownClassroom, Role.STUDENT);
        Subject withoutSchedule = subject("No schedule", ownClassroom);
        Subject otherSubject = subject("Other classroom", otherClassroom);
    ...
    private String bearer(User authenticatedUser) {
        return "Bearer " + jwtService.generate(authenticatedUser);
    }
    ...
    static class FixedClockConfig {
        ...
        Clock fixedClock() {
            return Clock.fixed(
                    ZonedDateTime.of(FIXED_DATE, LocalTime.of(10, 0), SAO_PAULO_ZONE).toInstant(),
                    SAO_PAULO_ZONE
            );
        }
    }
